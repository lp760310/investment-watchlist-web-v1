...
const SAFE_ALPHA_LAST_SUCCESS_NOTE = 'Alpha Vantage 本次请求失败或已达免费额度限制，当前显示上一次成功获取的数据。';
const SAFE_ALPHA_LOCAL_FALLBACK_NOTE = 'Alpha Vantage 当前不可用且暂无历史成功行情，当前显示观察池配置数据。';
const LAST_SUCCESS_FILE = path.join(__dirname, '..', 'data', 'us-market-last-success.json');
...
function ensureDataDir() {
  fs.mkdirSync(path.dirname(LAST_SUCCESS_FILE), { recursive: true });
}

function readUsLastSuccess() {
  try {
    if (!fs.existsSync(LAST_SUCCESS_FILE)) return {};
    const parsed = JSON.parse(fs.readFileSync(LAST_SUCCESS_FILE, 'utf8'));
...
function writeUsLastSuccess(updates) {
  const validUpdates = Object.entries(updates).filter(([, item]) => isValidUsQuote(item));
  if (validUpdates.length === 0) return;

  ensureDataDir();
  const current = readUsLastSuccess();
  for (const [symbol, item] of validUpdates) {
    current[symbol] = item;
  }
  fs.writeFileSync(LAST_SUCCESS_FILE, `${JSON.stringify(current, null, 2)}\n`);
}

function isValidUsQuote(row) {
  return Boolean(
    row &&
    row.type === '美股股票' &&
...
function buildLastSuccessQuote(asset, saved) {
  const row = {
    ...saved,
    symbol: asset.symbol,
    name: asset.name_cn || asset.name_en || asset.symbol,
    type: getType(asset),
    sectorOrTheme: saved.sectorOrTheme || asset.sector || null,
    dataSource: saved.dataSource || 'Alpha Vantage',
    dataStatus: '上次成功数据',
    dataDelayNote: SAFE_ALPHA_LAST_SUCCESS_NOTE
  };
...
async function fetchUsQuote(asset) {
  const lastSuccess = readUsLastSuccess();
  if (!process.env.ALPHA_VANTAGE_API_KEY) {
    if (lastSuccess[asset.symbol] && isValidUsQuote(lastSuccess[asset.symbol])) {
      return buildLastSuccessQuote(asset, lastSuccess[asset.symbol]);
    }
    return buildFallbackQuote(asset, SAFE_ALPHA_LOCAL_FALLBACK_NOTE);
  }
...
function buildSummary(items, fetchedAt, cacheInfo) {
  ...
  const usItems = items.filter((item) => item.type === '美股股票');
  const usStatuses = Array.from(new Set(usItems.map((item) => item.dataStatus)));
  let usMarketStatus = '本地兜底';
  if (usStatuses.length === 1 && usStatuses[0]) usMarketStatus = usStatuses[0];
  if (usStatuses.length > 1) usMarketStatus = '混合';
  ...
  return {
    ...
    usMarketStatus,
    usMarketNote: '美股免费接口额度有限，当前可能显示上一次成功数据。',
...
async function fetchUsItems(assets) {
  ...
  for (const asset of assets) {
    const row = await fetchUsQuote(asset);
    items.push(row);
    if (row.dataStatus === '接口数据' && isValidUsQuote(row)) {
      successUpdates[row.symbol] = row;
    }
    await sleep(250);
  }
  writeUsLastSuccess(successUpdates);
