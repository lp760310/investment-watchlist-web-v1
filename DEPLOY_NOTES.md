...
V2.4 的 `/api/market-data` 行情接口会优先尝试真实行情接口，并使用分市场内存缓存。美股接口失败、限流或字段缺失时，会优先显示 `data/us-market-last-success.json` 中保存的上一次成功行情；如果从未成功获取过，再回退到本地观察池配置。页面会显示数据来源、更新时间、行情状态和缓存设置。
...
- 只有成功获取到有效美股行情时，才会更新 `data/us-market-last-success.json`；失败、空字段或本地兜底不会覆盖旧成功数据。
